...
if [ ! -f "$MW_DIR/LocalSettings.php" ]; then
	...
	php "$MW_DIR/maintenance/install.php" \
		...
		--server="$MW_SERVER" \
		--scriptpath="" \
		"LabkiPageFormsInputsDev" \
		"$MW_ADMIN_USER"

	# SMW + dependencies must load BEFORE setupStore.php
...
EOF
	...
	# Our extension and any test-specific overrides load AFTER store setup,
	# the way a real admin would add an extension to an existing wiki.
	cat >> "$MW_DIR/LocalSettings.php" <<'EOF'
